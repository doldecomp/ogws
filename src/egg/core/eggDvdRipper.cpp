...
namespace EGG
{
        ...
        EAllocDirection allocDir, u32 pos, u32 *r8, u32 *pSize)
    {
        ...
        if (pos)
        {
            ...
            while (true)
            {
                result = DVDReadPrio(&dvdFile->mFileInfo, ut::RoundUp<void *>(buf, 32), 32, pos, DVD_QUEUE_PRIO_MEDIUM);
        ...
        while (true)
        {
            result = DVDReadPrio(&dvdFile->mFileInfo, dest, alignedSize - pos, pos, DVD_QUEUE_PRIO_MEDIUM);
