...
long double __strtold(int max_width, int (*ReadProc)(void*, int, int),
    ...
    *overflow = 0;
    ...
    while (count <= max_width && c != -1 && !final_state(scan_state)) {
        ...
        case 8192: {
            ...
            if ((i == 3) || (i == 4)) {
                if (i == 4) {
                    while ((j < 32) &&
                           (isdigit(c) || isalpha(c) || (c == radix_marker))) {
                        nan_arg[j++] = (char)c;
                        c = fetch();
                    }
    ...
    if (scan_state != 32768 ? !success(scan_state)
    ...
    if (hex_scan_state == 0) {
        ...
        // idk SHRT_MIN does not work...
        if (-0x8000 > exp_value || SHRT_MAX < exp_value) {
            *overflow = 1;
        }
        ...
        if (one_bit) {
            ...
            for (chptr = mantissa + sizeof(mantissa) - 1; chptr >= mantissa;
                 chptr--) {
...
double atof(const char* str, char** end) {
    double abs_value;
    double value;
    int overflow;
    int count;
    ...
    if (overflow ||
        (value != 0.0 && (abs_value < DBL_MIN || abs_value > DBL_MAX)))
        errno = ERANGE;
