    ...
    while (!terminate && (format_char = *format_ptr) != 0) {
            ...
            if (match_failure) {
                ...
                if ((format.argument_options == long_long_argument) ||
                    ...
                    u_long_long_num = __strtoull(
                        base, format.field_width, ReadProc, ReadProcArg,
                        &num_chars, &negative, &overflow);
                } else {
                    u_long_num = __strtoul(base, format.field_width, ReadProc,
                                           ReadProcArg, &num_chars, &negative,
                                           &overflow);
                }
                ...
                if ((format.argument_options == long_long_argument) ||
                    (format.argument_options == intmax_argument)) {
                    long_long_num =
                        (negative ? -u_long_long_num : u_long_long_num);
                } else {
        ...
        flt:
            if (match_failure) {
                ...
                long_double_num = __strtold(format.field_width, ReadProc,
                                            ReadProcArg, &num_chars, &overflow);
            ...
            if (arg_ptr) {
                ...
                if (match_failure) {
                    ...
                    while (format.field_width-- &&
                           (!is_secure ||
                            ((elem_valid = (elem_maxsize > num_chars)) != 0)) &&
                           ((rval = ((*ReadProc)(ReadProcArg, 0,
                                                 __GetAChar))) != EOF)) {
                        c = rval;
