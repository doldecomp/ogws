...
long _ftell(FILE* pFile) {
    ...
    if (!(file_kind == file_disk || file_kind == file_console) ||
        pFile->state.error) {
        errno = 40;
        return -1;
    }
...
int _fseek(FILE* pFile, long offs, int file_mode) {
    ...
    if ((file_mode != 2) && (pFile->mode.io != 3) &&
        ((pFile->state.io_state == reading) ||
         (pFile->state.io_state == rereading))) {
    ...
    if (pFile->state.io_state == neutral) {
        ...
        if (position_proc_func != 0 &&
            (*position_proc_func)(pFile->handle, &offs, file_mode,
                                  pFile->ref)) {
            pFile->state.error = 1;
            pFile->buffer_len = 0;
            errno = 40;
