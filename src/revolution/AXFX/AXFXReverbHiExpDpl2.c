...
static BOOL __AllocDelayLine(AXFX_REVERBHI_EXP_DPL2* fx);
static void __BzeroDelayLines(AXFX_REVERBHI_EXP_DPL2* fx);
static void __FreeDelayLine(AXFX_REVERBHI_EXP_DPL2* fx);
...
static u32 __FilterSizeTable[7][9] = {
...
BOOL AXFXReverbHiExpInitDpl2(AXFX_REVERBHI_EXP_DPL2* fx) {
    ...
    if (fx->preDelayTimeMax < 0.0f) {
        AXFXReverbHiExpShutdownDpl2(fx);
        OSRestoreInterrupts(mask);
        return FALSE;
    }
...
void AXFXReverbHiExpCallbackDpl2(AXFX_BUFFERUPDATE_DPL2* bufferUpdate,
    ...
    input[2] = bufferUpdate->left_surround;
    input[3] = bufferUpdate->right_surround;

    if (fx->busIn != NULL) {
        inBusData[0] = fx->busIn->left;
        inBusData[1] = fx->busIn->right;
        inBusData[2] = fx->busIn->left_surround;
        inBusData[3] = fx->busIn->right_surround;
    }

    if (fx->busOut != NULL) {
        outBusData[0] = fx->busOut->left;
        outBusData[1] = fx->busOut->right;
        outBusData[2] = fx->busOut->left_surround;
        outBusData[3] = fx->busOut->right_surround;
    }
    ...
    crosstalkGain = fx->crosstalk * 0.333333f;
    ...
    for (samp = 0; samp < 96; samp++) {
        ...
        crosstalkL = fusedOut[1] + fusedOut[2] + fusedOut[3];
        crosstalkR = fusedOut[0] + fusedOut[2] + fusedOut[3];
        crosstalkLS = fusedOut[0] + fusedOut[1] + fusedOut[3];
        crosstalkRS = fusedOut[0] + fusedOut[1] + fusedOut[2];

        output[0] = fusedOut[0] + crosstalkL * crosstalkGain;
        output[1] = fusedOut[1] + crosstalkR * crosstalkGain;
        output[2] = fusedOut[2] + crosstalkLS * crosstalkGain;
        output[3] = fusedOut[3] + crosstalkRS * crosstalkGain;

        *(input[0]++) = (s32)(output[0] * fx->outGain);
        *(input[1]++) = (s32)(output[1] * fx->outGain);
        *(input[2]++) = (s32)(output[2] * fx->outGain);
        *(input[3]++) = (s32)(output[3] * fx->outGain);
...
DECOMP_FORCELITERAL(AXFXReverbHiExpDpl2_c, -3.0f, 10.0);
