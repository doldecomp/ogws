...
void AnmObjVisNode::SetUpdateRate(f32 rate) {
    for (int i = 0; i < MAX_RESOURCES; i++) {
        if (mResources[i] != NULL)
            mResources[i]->SetUpdateRate(rate);
    }
}
...
f32 AnmObjVisNode::GetUpdateRate() const {
    for (int i = 0; i < MAX_RESOURCES; i++) {
        if (mResources[i] != NULL)
            return mResources[i]->GetUpdateRate();
    }
...
AnmObjVisOR* AnmObjVisOR::Construct(MEMAllocator* allocator, u32* pSize,
                                    ResMdl mdl) {
    if (!mdl.IsValid())
        return NULL;
...
AnmObjVisRes* AnmObjVisRes::Construct(MEMAllocator* allocator, u32* pSize,
...
AnmObjVisRes::AnmObjVisRes(MEMAllocator* allocator, ResAnmVis vis,
                           u16* bindBuffer, int numBinds)
